...
"""usage: {} [-h --help] [-v --verbose] [-d --dir </path/to/directory>] [-f --file <path/to/file>]
Args:
...
def scan_dir(path: str) -> Optional[int]:
    """
    ...
    for i in os.listdir(path):
        if count := scan_file(os.path.join(path, i)):
            _sum += count
...
def main():
    ...
    _sum_lines += iterate(dir_entries, scan_dir, verbose)
    if verbose:
